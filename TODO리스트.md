...
- [ ] **Play Console 내부 테스트에 새 버전 배포**
  - Play Console > **테스트 및 출시 > 내부 테스트** > 새 버전 만들기
  - 새 `app-release.aab` 업로드, 출시 이름·변경사항 입력, 검토 후 내부 테스터에게 출시
  - 출시 상태가 **활성 / 내부 테스터에게 제공됨**인지 확인
  - versionCode가 기존 1보다 큰지 확인
- [ ] **내부 테스터 연결 및 설치 확인**
  - 내부 테스트 > **테스터** 탭에서 이메일 목록을 만들거나 수정한다.
  - 같은 화면의 **테스터 표에서 해당 이메일 목록을 선택**하고 변경사항을 저장한다. 이메일 목록에 주소가 있는 것만으로는 트랙 대상이 되지 않는다.
  - Play Console이 그 화면에서 생성한 공유 링크를 그대로 사용한다. URL을 수동으로 만들거나 끝부분을 수정하지 않는다.
  - 링크를 여는 브라우저와 테스트 기기의 Play Store가 초대된 동일한 Google 계정으로 로그인됐는지 확인한다.
  - 참여 후 Play Store에서 설치·업데이트가 가능한지, 설치된 앱의 버전이 이번 AAB인지 확인한다.
