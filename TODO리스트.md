...
- [ ] **Play Console 내부 테스트에 새 AAB 업로드**
  - 기존 출시를 수정하는 대신 `versionCode 5` AAB로 새 출시를 만든다.
  - 변경 내용에 `소셜 로그인 안정화, 앱 용량 최적화, 출시 안정성 개선`을 기록한다.
  - 처리 완료 후 초대된 Google 계정으로 Play 스토어에서 업데이트한다.

- [ ] **카카오 로그인 실기기 검증**
  - 카카오톡 설치 상태와 미설치/웹 로그인 상태를 각각 확인한다.
  - 실패하면 Snackbar 끝의 `[KAKAO/...]` 코드를 그대로 기록해 전달한다. 액세스 토큰이나 앱 키는 전달하지 않는다.
  - `KAKAO/misconfigured`, `KAKAO/invalidClient`, `KAKAO/unauthorized`이면 Kakao Developers의 해당 Native App Key에서 다음을 다시 확인한다.
    - Android 패키지명 `kr.hehehe.hehe`
    - 카카오 로그인 활성화
    - Play **앱 서명 키 인증서**의 키 해시 등록(업로드 키가 아님)

- [ ] **네이버 로그인 실기기 검증**
  - Naver Developers에서 Android 패키지명 `kr.hehehe.hehe`와 Client ID/Secret을 확인한다.
  - 서비스가 `개발 중`이면 로그인에 사용할 네이버 ID가 테스트 ID로 등록되어 있는지 확인한다.
  - 실패하면 Snackbar 끝의 `[NAVER/...]` 코드만 기록해 전달한다.
  - 내부 검증이 끝나면 프로덕션 출시 전에 네이버 로그인 서비스 적용/검수를 요청한다.
